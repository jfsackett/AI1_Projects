...
/** Solves puzzle by using heuristic which minimizes # moves to state + tiles' total distance out of place. */
public class AstarMinDistanceSearchStrategy implements SearchStrategy {
	
	/** Perform A* search by minimizing # moves to state + tiles' total distance out of place and return results info. */
	...
	private static void insertIntoHeuristicSortedLoc(PuzzleMove newPuzzleMove, PuzzleState goalPuzzleState, LinkedList<PuzzleMove> frontierPuzzleMoves) {
		...
		AstarMinDistanceVisitor visitor = new AstarMinDistanceVisitor(newPuzzleMove, goalPuzzleState, frontierPuzzleMoves.size());
